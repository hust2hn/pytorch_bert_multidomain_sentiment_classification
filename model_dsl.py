...
parser.add_argument("--layer_num", type=int, default=12, help="指定层")
...
class BertClassifier(nn.Module):
    def __init__(self, dropout=0.9):
        ...
        self.s_linear = nn.Linear(768*2, 2)
        self.s_relu = nn.ReLU()

    def forward(self, input_id, mask):
        _, pooled_output, all_hidden_states, _ = self.bert(input_ids=input_id, attention_mask=mask, return_dict=False)

        d_dropout_output = self.d_dropout(torch.mean(all_hidden_states[args.layer_num], dim=1))
        d_linear_output = self.d_linear(d_dropout_output)
        d_final_layer = self.d_relu(d_linear_output)

        s_dropout_output = self.s_dropout(pooled_output)
        s_linear_output = self.s_linear(s_dropout_output)
        s_final_layer = self.s_relu(s_linear_output)
        return s_final_layer, d_final_layer
...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    for epoch_num in range(epochs):
        ...
        for train_input, train_label, s_train_label in tqdm(train_dataloader):
            ...
            output, d_output = model(input_id, mask)

            batch_loss = criterion1(output, s_train_label.long())
            total_loss_train += batch_loss.item()

            d_batch_loss = criterion2(d_output, train_label.long())
            d_total_loss_train += d_batch_loss.item()

            acc1 = (output.argmax(dim=1) == s_train_label).sum().item()
            acc2 = (d_output.argmax(dim=1) == train_label).sum().item()

            total_acc_train += acc1
            d_total_acc_train += acc2

            batch_loss = batch_loss + args.coef * d_batch_loss
            model.zero_grad()
            batch_loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for val_input, val_label, s_val_label in val_dataloader:
                ...
                batch_loss = criterion1(output, s_val_label.long())
                d_batch_loss = criterion2(d_output, val_label.long())
                total_loss_val += batch_loss.item()
                d_total_loss_val += d_batch_loss.item()

                acc1 = (output.argmax(dim=1) == s_val_label).sum().item()
                acc2 = (d_output.argmax(dim=1) == val_label).sum().item()
                total_acc_val += acc1
                d_total_acc_val += acc2
        print(
...
def evaluate(model, test_data):
    ...
    with torch.no_grad():
        for test_input, test_label, s_test_label in test_dataloader:
            ...
            acc = (output.argmax(dim=1) == s_test_label).sum().item()
            d_acc = (d_output.argmax(dim=1) == test_label).sum().item()
            total_acc_test += acc
            d_total_acc_test += d_acc
...
torch.save(model, 'model_dsl-' + str(args.seed) + '-' + str(args.coef) + str(args.num_domain) + '-' + str(args.layer_num) + '.pth')
